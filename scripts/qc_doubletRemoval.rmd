...
for (sample in samples){
  ...
  # pathing
  path_cellbender <- paste0(paste0(DATA_ROOT, "/cellbender/breast_cancer/", patient, "/", sample, "_cellbender_filtered.h5")
  path_cellranger <- paste0(paste0(DATA_ROOT, "/cellbender/breast_cancer/", patient, "/", sample, "_filtered_feature_bc_matrix.h5")
  ...
  saveRDS(object, paste0(paste0(DATA_ROOT, "/processed_data/breast_cancer/single_nuclei/per_sample/"), patient, "/", sample, ".rds"))
}
```

## Merge samples 
```{r}
samplePath <- paste0(DATA_ROOT, "/processed_data/breast_cancer/single_nuclei/per_sample/")
spls <- list()
...
c1 <- p1 - p2
ggsave(paste0(paste0(DATA_ROOT, "/figs/breast_cancer/qc_doubletRemoval/", patient, "/", patient,"_qc.pdf"), c1, width = 30, height = 10)
```
...
ggsave(paste0(paste0(DATA_ROOT, "/figs/breast_cancer/qc_doubletRemoval/", patient, "/clustering/", patient, "_elbow.pdf"), p, width = 10, height = 10)
```
...
wd <- paste0(paste0(DATA_ROOT, "/figs/breast_cancer/qc_doubletRemoval/", patient, "/clustering/")
...
# Save n load
```{r}
saveRDS(spl.filt, paste0(paste0(DATA_ROOT, "/processed_data/breast_cancer/single_nuclei/per_sample/"), patient, "/", patient, "_merged.rds"))
```
```{r}
spl.filt <- readRDS(paste0(paste0(DATA_ROOT, "/processed_data/breast_cancer/single_nuclei/per_sample/"), patient, "/", patient, "_merged.rds"))
```
...
```{r doubletfinder}
wd <- paste0(paste0(DATA_ROOT, "/figs/breast_cancer/qc_doubletRemoval/", patient, "/doublets/")
if (!dir.exists(wd)) {
...
```{r}
saveRDS(data_list, paste0(paste0(DATA_ROOT, "/processed_data/breast_cancer/single_nuclei/per_sample/"), patient, "/", patient, "_merged_postDoublet.rds"))
```
...
ggsave(paste0(paste0(DATA_ROOT, "/figs/breast_cancer/qc_doubletRemoval/", patient, "/clustering/", patient, "elbow_noDoublets.pdf"), p, width = 10, height = 10)
```
...
saveWidget(p, file = paste0(DATA_ROOT, "/figs/breast_cancer/qc_doubletRemoval/pre_annotation_singleNuclei/clustering/umap_harmony_3D.html"))
```


## Save object
```{r save_RDS}
saveRDS(spl.filt_noDoublets, file = paste0(paste0(DATA_ROOT, "/processed_data/breast_cancer/single_nuclei/per_sample/"), patient, "/", patient, "_noDoublets.rds"))
```
