...
## Load libraries
Use the `simplex` conda env (`conda activate simplex`); DoubletFinder >= 2.0.6 is pinned in `renv.lock` and works with Seurat v5.
...
for (sample in samples){
  ...
  # pathing (HD patient4 = two libraries under patient4/patient4_1, patient4/patient4_2)
...
data_list <- lapply(datasets, function(x) {
  ...
  BC_pK <- as.numeric(as.character(pK[[1]])) # use a single numeric value as pK parameter in doubletFinder()
  ...
  subset <- doubletFinder(subset, pN = 0.25, pK = BC_pK, nExp = nExp, PCs = 1:30, sct = TRUE)
...
## Doublet removal
```{r filter.doublets}
df_col <- grep("^DF\\.classifications", colnames(spl.doub@meta.data), value = TRUE)
stopifnot(length(df_col) == 1L)
table(spl.doub@meta.data[[df_col]])
spl.filt_noDoublets <- spl.doub[, spl.doub@meta.data[[df_col]] == "Singlet"]
```
