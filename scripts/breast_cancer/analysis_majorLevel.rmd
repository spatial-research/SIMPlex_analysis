...
### Merge datasets
```{r}
data_path <- paste0(DATA_ROOT, "/r_objects/breast_cancer/single_nuclei/per_sample")
patient_csv <- read.csv(here::here("resources/sample_metadata.csv"), sep = ";")
included_samples <- c(
    "patient1_55um", 
    "patient2_55um", 
    "patient4_55um", 
...
for (sample in samples) {
    output_path <- paste0(paste0(DATA_ROOT, "/r_objects/breast_cancer/visium/55um/", sample, "/")
    if (grepl("HD", sample)) {
        for (assay in assays) {
...
dir.create(paste0(paste0(DATA_ROOT, "/r_objects/breast_cancer/visium/HD/"), sample, "_visHD/"), recursive = TRUE, showWarnings = FALSE)
saveRDS(se.hd, paste0(paste0(DATA_ROOT, "/r_objects/breast_cancer/visium/HD/"), sample, "_visHD/", sample, "_visHD.rds"))
}
```
...
```{r}
int.all <- readRDS(paste0(DATA_ROOT, "/r_objects/breast_cancer/single_nuclei/cross_patient/SIMPlex_breast_allSamples.rds"))
```
...
#### load 
```{r}
qc <- readRDS(paste0(DATA_ROOT, "/r_objects/breast_cancer/single_nuclei/cross_patient/SIMPlex_breast_allSamples.rds"))
qc$sample <- paste0("BC_", qc$sample)
ff <- readRDS(paste0(DATA_ROOT, "/r_objects/breast_cancer/single_nuclei/other/patient10_seuratAnnotation.rds"))
ff$sample[ff$sample == "BC_1.1"] <- "BC_FreshFrozen_rep1"
ff$sample[ff$sample == "BC_1.2"] <- "BC_FreshFrozen_rep2"
prost1 <- readRDS(paste0(DATA_ROOT, "/r_objects/prostate_cancer/single_nuclei/pt10_HD_seuratAnnotation.rds"))
prost1$sample[prost1$sample == "pt10_ap5_5"] <- "Prostate_HD_pat10"
prost2 <- readRDS(paste0(DATA_ROOT, "/r_objects/prostate_cancer/single_nuclei/pt20_HD_seuratAnnotation.rds"))
...
mouse <- readRDS(paste0(DATA_ROOT, "/r_objects/mouse_brain/snRNA.rds"))
