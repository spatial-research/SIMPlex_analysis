...
## Cell type representation versus garvan Atlas / others

#### load others 
```{r}
garvan <- readRDS(paste0(DATA_ROOT, "/external_references/BC_atlas/miniatlas.rds"))
# merge epithelial layers
garvan@meta.data$celltype_major <- ifelse(
    garvan@meta.data$celltype_major %in% c("Normal Epithelial", "Cancer Epithelial"),
    "Epithelial",
    garvan@meta.data$celltype_major
)
```

```{r}
int_counts <- table(int.all$seuratAnnotation_major)
garvan_counts <- table(garvan$celltype_major)

data <- data.frame(
  CellType = c(names(int_counts), names(garvan_counts)),
  Count = c(as.numeric(int_counts), as.numeric(garvan_counts)),
  Dataset = rep(c("SIMPlex", "BC Atlas"), times = c(length(int_counts), length(garvan_counts))),
  N = rep(c(length(unique(int.all$sample)), length(unique(garvan$Patient))), 
            times = c(length(int_counts), length(garvan_counts)))
)
...
p1 <- ggplot(data) + 
    ...
    annotate("text", x = 1, y = 1.05, label = paste0("N=", length(unique(garvan$Patient)), "\nNcells=", length(garvan$Patient)), size = 3) +
...
#### Map Garvan Atlas to Spatial data
```{r}
samples <- unique(int.all$sample)
...
### 3D correlation of SIMPlex, Garvan Atlas and Control
...
### --- A) Garvan scRNA --- ###
garvan_counts <- table(garvan$celltype_major)
garvan_props <- data.frame(
    CellType = names(garvan_counts),
    Proportion = as.numeric(garvan_counts) / sum(garvan_counts),
    Dataset = "Garvan_scRNA"
)
...
### --- C) Spatial Deconvolution (patspec + garvan) --- ###
patspec_mat <- mergedVis_pathAnno@assays$patspec_major_map@data
garvan_mat  <- mergedVis_pathAnno@assays$garvan_major_map@data

patspec_counts <- rowSums(patspec_mat)
garvan_counts_sp <- rowSums(garvan_mat)
...
garvan_sp_props <- data.frame(
    CellType = names(garvan_counts_sp),
    Proportion = garvan_counts_sp / sum(garvan_counts_sp),
    Dataset = "Spatial_garvan"
)
...
all_props$Dataset <- dplyr::recode(
  all_props$Dataset,
  "Garvan_scRNA"   = "BC Atlas",
  "Pathology"      = "Histopathology",
  "SingleNuclei"   = "SIMPlex",
  "Spatial_garvan" = "Deconv. BC Atlas",
...
garvan$major_celltypes <- garvan$celltype_major
refs <- list(garvan = garvan, SIMPlex = simp, control = control)
...
for (res in ress) {
    ...
    for (refName in c("garvan", "SIMPlex", "control")){
        ref <- refs[[refName]]
        DefaultAssay(subRes) <- "Spatial"
...
garvan_mat  <- mergedVis@assays$garvan_major_map@data

# Sum across cells for each cell type 
patspec_counts <- rowSums(patspec_mat)
garvan_counts  <- rowSums(garvan_mat)

# Combine into data.frame
data <- data.frame(
    CellType = c(names(patspec_counts), names(garvan_counts)),
    Count = c(as.numeric(patspec_counts), as.numeric(garvan_counts)),
    Dataset = rep(c("SIMPlex", "BC Atlas"), 
                                times = c(length(patspec_counts), length(garvan_counts)))
)
