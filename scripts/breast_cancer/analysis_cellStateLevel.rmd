...
wd <- paste0(DATA_ROOT, "/figs/breast_cancer/analysis_cellStateLevel/")
...
## Integrate CAFs only 
```{r}
wd <- paste0(DATA_ROOT, "/figs/breast_cancer/analysis_cellStateLevel/subtype_annotation/")
...
WE compare gene expression to our Reference atlas from Garvan
...
# Settings
```{r}
wd <- paste0(DATA_ROOT, "/figs/breast_cancer/analysis_cellStateLevel/final_subpopulations/")
```

## load 
```{r}
int.all <- readRDS(paste0(rootObj, "integrated_all_subpopulations.rds"))
mergedVis <- readRDS(paste0(rootObj, "merged_visium_deconSubpopulations_simplex_garvan.rds"))
```
...
# saveRDS(mergedVis, paste0(rootObj, "merged_visium_deconSubpopulations_simplex_garvan.rds"))
```
...
#### manually plot umaps 
```{r}
spl <- readRDS(paste0(DATA_ROOT, "/processed_data/breast_cancer/single_nuclei/cross_patient/caf_subpop_analysis.rds"))
...
```{r}
library(igraph)
wd <- paste0(DATA_ROOT, "/figs/breast_cancer/analysis_cellStateLevel/ref_atlas_comparisons/")
int.all <- readRDS(paste0(rootObj, "integrated_all_subpopulations.rds"))
...
```{r}
samples <- unique(int.all$sample[!grepl("HD", int.all$sample)])
garvan$celltype_subset <- gsub("[ _]", "-", garvan$celltype_subset)
features = unique(garvan$celltype_subset)
for (sample in samples){
    vis <- spls[[sample]]
    patient_ID <- sub("_.*$", "", sample)
    ref <- garvan
...
# Generate heatmap and save as PDF
pdf(paste0(wd, "correlation/correlation_garvanSubset.pdf"), width = 13, height = 10)
heatmap3(
    cor_matrix, 
    margins = c(14, 14), 
    symm = TRUE,
...
```{r}
wd <- paste0(DATA_ROOT, "/figs/breast_cancer/analysis_cellStateLevel/HD/")
```
...
garvan$subpopulation <- garvan$celltype_subset
refs <- list(garvan = garvan, SIMPlex = simp)
...
for (res in ress) {
    ...
    for (refName in c("garvan", "SIMPlex")){
...
## load 
```{r}
int.all <- readRDS(paste0(DATA_ROOT, "/processed_data/breast_cancer/single_nuclei/cross_patient/integrated_all_subpopulations.rds"))
mergedVis <- readRDS(paste0(rootObj, "merged_visium_deconSubpopulations_simplex.rds"))
rootObj <- paste0(DATA_ROOT, "/processed_data/breast_cancer/single_nuclei/cross_patient/")
wd <- paste0(DATA_ROOT, "/figs/breast_cancer/analysis_cellStateLevel/pat4_invasive_dcis_composition/")
...
```{r}
wd <- paste0(DATA_ROOT, "/figs/breast_cancer/analysis_cellStateLevel/deg_interesting_celltypes/")   
```
...
library(celltalker)
int.all <- readRDS(paste0(DATA_ROOT, "/processed_data/breast_cancer/single_nuclei/cross_patient/integrated_all_subpopulations.rds"))
wd <- paste0(DATA_ROOT, "/figs/breast_cancer/analysis_cellStateLevel/ligand_receptor/")
```
...
```{r}
mtx <- read.csv(paste0(DATA_ROOT, "/figs/breast_cancer/analysis_cellStateLevel/final_subpopulations/correlation/patient4_55um_correlation_matrix.csv", row.names = 1, check.names = FALSE)
...
```{r}
wd <- paste0(DATA_ROOT, "/figs/breast_cancer/analysis_cellStateLevel/interesting_genes/")
mergedVis <- readRDS(paste0(rootObj, "merged_visium_deconSubpopulations_simplex.rds"))
```
...
# Looking for FDCs
```{r}
wd <- paste0(DATA_ROOT, "/figs/breast_cancer/analysis_cellStateLevel/fdc_frc_analysis/")
int.all <- readRDS(paste0(DATA_ROOT, "/processed_data/breast_cancer/single_nuclei/cross_patient/integrated_all_subpopulations.rds"))
mergedVis <- readRDS(paste0(DATA_ROOT, "/processed_data/breast_cancer/visium/cross_patient/merged_visium_deconSubpopulations_simplex.rds"))
```

### FDC
```{r}
wd <- paste0(DATA_ROOT, "/figs/breast_cancer/analysis_cellStateLevel/fdc/")
...
mergedVis@tools$Staffli@imgs <- c(
  "patient1_55um" = paste0(DATA_ROOT, "/spaceranger/breast_cancer/55um/3925_1K_R1S/spatial/tissue_lowres_image.png",
  "patient2_55um" = paste0(DATA_ROOT, "/spaceranger/breast_cancer/55um/4286-2_4F_R1S/spatial/tissue_lowres_image.png",
  "patient4_55um" = paste0(DATA_ROOT, "/spaceranger/breast_cancer/55um/3962_3H_R1L/spatial/tissue_hires_image.png",
  "patient5_55um" = paste0(DATA_ROOT, "/spaceranger/breast_cancer/55um/4497-1_2K_R1L/spatial/tissue_hires_image.png",
  "patient6_55um" = paste0(DATA_ROOT, "/spaceranger/breast_cancer/55um/4754_1G_R1L/spatial/tissue_hires_image.png",
  "patient7_55um" = paste0(DATA_ROOT, "/spaceranger/breast_cancer/55um/3585_1K_R1L/spatial/tissue_hires_image.png",
  "patient8_55um" = paste0(DATA_ROOT, "/spaceranger/breast_cancer/55um/4850_3F_R1S/spatial/tissue_hires_image.png",
  "patient9_55um" = paste0(DATA_ROOT, "/spaceranger/breast_cancer/55um/4246_3E_R1L/spatial/tissue_hires_image.png"
)
