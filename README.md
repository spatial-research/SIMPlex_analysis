...
├── data/                          # ALL HEAVY DATA LIVES HERE — gitignored. See data/<dir>/PLACEHOLDER.md
└── docs/
...
Patients 4 and 5 are the deeply multimodal TNBC cases (Visium 55 µm + Visium HD + snRNA + consecutive Xenium). Internal Garvan IDs map to public patient IDs via `resources/patient_metadata.csv`.
...
```bash
git clone <repo-url> SIMPlex_analysis
cd SIMPlex_analysis
git init                            # required for here::here() to anchor on this directory
# (or `touch .here` if you prefer not to use git)
```
...
`data/` is gitignored and contains all heavy inputs and outputs. After cloning, `data/processed_data/` is already populated with all deposited R objects (~37 GB, mirrors the KTH deposit layout). Run the structure check to see what other categories still need populating:

```bash
find data -name PLACEHOLDER.md
```

Each PLACEHOLDER explains what files belong there. The directory layout is:

```
data/
├── raw_data/                   # FASTQs + H&E images (placeholders; not yet deposited)
├── spaceranger/                # SpaceRanger outputs (Visium 55 µm, HD, mouse brain) — partially populated, see PLACEHOLDERs
├── cellranger/                 # CellRanger snRNA-seq outputs (TO BE ADDED from collaborators)
├── cellbender/                 # CellBender-denoised h5 matrices (input to qc_doubletRemoval.rmd)
├── xenium/                     # Full Xenium Analyzer outputs (zarr/zip) — also available under processed_data/breast_cancer/xenium/
├── external_references/        # Public atlases (Wu BC, Linnarsson, Allen, Cords CAF, snPATHO, 10k mouse forebrain, PCa atlas)
├── processed_data/             # All R objects + processed CSVs — populated from KTH deposit (37 GB)
└── figs/                       # Generated by every script under figs/<provenance>/<analysis>/
```

See `docs/data_availability.md` for the full inventory of what is present, what is missing, and how to obtain each missing piece.
